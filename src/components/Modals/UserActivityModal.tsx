...
import { getUserActivities } from '@/Redux/Home/Slice';
...
const UserActivityModal: FC<Props> = ({ close }) => {
  const { userActivities, userActivitiesPagination, loading } = useAppSelector((state) => state.homePage);
  const dispatch = useAppDispatch();

  useEffect(() => {
    dispatch(getUserActivities({ page: userActivitiesPagination?.page ?? 1, limit: userActivitiesPagination?.limit ?? 10 }));
  }, [dispatch]);
  return (
    <div>
      <Modal classNames={`md:max-w-[40%] md:min-w-[40%] h-fit overflow-x-auto scrollbar-thin`} closeModal={close}>
        <Fragment>
          ...
          >
            <div>
              <h1 className="text-heading font-semibold">Latest User Activities</h1>
          ...
          <Modalbody fixedHeight={false}>
            <div>
              ...
              <div className="mt-4">
                <CardPagination
                  key={userActivitiesPagination?.totalPages}
                  showItems={5}
                  totalPages={userActivitiesPagination?.totalPages}
                  onPageChange={(page: number) => dispatch(getUserActivities({ page, limit: userActivitiesPagination.limit }))}
                />
              </div>
            </div>
          </Modalbody>
