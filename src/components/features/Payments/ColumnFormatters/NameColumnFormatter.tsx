  ...
  return (
    <div className="flex w-[200px] gap-5 py-2 items-center">
      ...
      <div className="text-sm font-semibold">
        <div>
          <span className="text-paragraph font-bold text-nowrap">{row.name && row.name !== '' ? truncateText(row.name, 50) : '---'}</span>
        </div>
        <div>
          <span className="text-gray-700">{row.phone && row.phone !== '' ? truncateText(row.phone, 15) : '---'}</span>
        </div>
      </div>
    </div>
  );
