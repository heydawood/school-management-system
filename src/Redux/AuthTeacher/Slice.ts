...
import { teacherLoginHandler } from '@/Services/AuthTeacher/AuthTeacher';
...
// Thunks
export const teacherLogin = createAsyncThunk('auth/teacherLogin', async (data: { email: string; password: string }, { rejectWithValue }) => {
  try {
    const response = await teacherLoginHandler(data.email, data.password);
...
export const { setToken, setTeacherInfo, setEmail, setOtp, clearForgotFlow, setRole } = authTeacherSlice.actions;
